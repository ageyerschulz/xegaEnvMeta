...
#' Factory for function F4det (30-dimensional quartic without noise)
#'
#' @description This function factory sets up the problem environment
#'              for De Jong's function F4 but without noise.
#' F4det is a 30-dimensional quartic function with Gaussian noise.
#' It is a continuous, convex, unimodal, high-dimensional quartic function
#' Note: There exist \eqn{2^{30}} maxima (without noise)!
#'
#' @details The implementation works for a n-dimensional version of F4det.
#'
...
#'
#' @examples
#' DeJongF4det<-DeJongF4Factory()
#' DeJongF4det$name()
#' DeJongF4det$bitlength()
#' DeJongF4det$genelength()
#' DeJongF4det$lb()
#' DeJongF4det$ub()
#' DeJongF4det$f(c(2.01, -1.05, 4.95, -4.3, -3.0))
#' DeJongF4det$f(c(2.01, -1.05, 4.95, -4.3, -3.0))
#' DeJongF4det$describe()
#' DeJongF4det$solution()
#' @importFrom stats rnorm
#' @export
DeJongF4detFactory<-function() {
self<-list()
self$name<-function() {"DeJongF4det"}
