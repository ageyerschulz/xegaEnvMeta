...
#' Factory for optimizing the 5 basic parameters 
#' of a standard genetic algorithm with a limited number of evaluations
#'
...
#' @param maxEvals   Maximal number of evaluations (resource limit).
...
#' bl<-rep(64,4); lb<-c(rep(10, 1), rep(0.00001, 3)); ub<-c(rep(15, 1), rep(1.0, 3))
#' stepsT<-15
#' path<-xegaTmpDir()
#' P5LR<-P5LRmetaGAFactory(EnvList, name="P5Shekel5d", maxEvals=5000,
#'     pnames=pn, bitlength=bl, lb=lb, ub=ub, stepsT=stepsT,
#'     repExp=1, log=1, path=path)
#' a<-xegaRun(penv=P5LR, algorithm="sga", max=TRUE, 
...
#' @export
P5LRmetaGAFactory<-function(envList, 
...
self$maxEvals<-xegaSelectGene::parm(maxEvals)
...
self$InnerGASolution<-xega::xegaRun(
             ...
             generations=as.integer(self$maxEvals()/as.integer(self$lb()[1])),
             mutrate=self$lb()[2], 
             bitmutrate=self$lb()[3],
             crossrate=self$lb()[4], 
...
self$f=function(parm, gene=0, lF=0) 
	{
		...
		popsize<-as.integer(parm[1])
                generations<-as.integer(self$maxEvals()/parm[1])
		mutrate<-parm[2]
		bitmutrate<-parm[3]
		crossrate<-parm[4]
...
t<-self$lb(); t<-self$ub(); t<-self$stepsT(); t<-self$repExp(); t<-self$maxEvals(); 
